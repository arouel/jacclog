...
public class OperatingSystemTest {
	...
	@Test
	public void testOperatingSystemBuilding() throws Exception {
		...
		builder.setUrl("url");
		builder.setPatternSet(new TreeSet<OperatingSystemPattern>());
		OperatingSystem browser = builder.build();
		Assert.assertEquals("family", browser.getFamily());
		Assert.assertEquals("icon", browser.getIcon());
		Assert.assertEquals("name", browser.getName());
		Assert.assertEquals(new TreeSet<OperatingSystemPattern>(), browser.getPatternSet());
