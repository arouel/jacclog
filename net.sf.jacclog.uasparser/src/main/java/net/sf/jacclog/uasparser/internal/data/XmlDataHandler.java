...
public class XmlDataHandler extends DefaultHandler {
	...
	public enum Tag {
		...
		public static Tag evaluate(final String tagName) {
			if (tagName == null) {
				throw new IllegalArgumentException("Argument 'tagName' can not be null.");
			}
	...
	private void transferToSpecificBuilderAndReset() {
		...
		if (isBrowserPattern && currentTag == Tag.PATTERN_ORDER) {
			browserPatternBuilder.setOrder(buffer.toString());
			...
			browserPatternBuilder.setPattern(buffer.toString());
		}
		...
		if (isOperatingSystemPattern && currentTag == Tag.PATTERN_ORDER) {
			operatingSystemPatternBuilder.setOrder(buffer.toString());
			...
			operatingSystemPatternBuilder.setPattern(buffer.toString());
		}
